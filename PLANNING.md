...
- [ ] **P6.6+** Schedule/Activity/Automations · proactive flows per tenant · per-tenant WhatsApp channel (WAHA multi-session) · Leads · self-review
...
## P6.6+ (later, one at a time)
Schedule/Activity/Automations (needs a per-tenant scheduler + delivery channel — the APScheduler-less poll loop in `scheduled_jobs.py` only runs in the owner entrypoint; this also unblocks real Push delivery for tenants); per-tenant proactive flows (the flow engine in `proactive/flows.py` is owner-only, same constraint); **per-tenant WhatsApp channel** (WAHA multi-session — one WA session per tenant, webhook resolves session→tenant scope); **per-tenant Leads**; **per-tenant self-review**.
