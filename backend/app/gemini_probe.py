...
# Tried in order, strongest first, but "lite" tiers are tried before their
# full counterpart at each rung — Flash-Lite has been the cheaper variant
# every generation so far, so a restricted/free-tier account settles on the
# cheaper option rather than the first thing that happens to work.
#
...
#
# This is the hard ceiling for automatic selection — never exceeded unless a
# tenant opts in (see PREMIUM_MODELS).
STANDARD_MODELS = [
